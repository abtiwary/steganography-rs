//! A command line application to drive the encoder and decoder for PNG images
...
fn main() -> Result<()> {
    ...
    if let Some(png_file) = matches.get_one::<String>("input-file") {
        ...
        info!(logger, "going to write this output file with the encoded message: {}", &out_file_path);
